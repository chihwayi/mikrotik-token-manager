...
class MockRouterOSServer {
    ...
    this.server = net.createServer((socket) => {
      ...
      socket.on('data', (data) => {
        ...
        while (buffer.length >= 4) {
          ...
          this.handleMessage(socket, messageBuffer, () => authenticated, (auth) => {
            authenticated = auth;
  ...
  handleMessage(socket, messageBuffer, isAuthenticated, setAuth) {
    ...
    if (command === '/login') {
      ...
      if (!username || !password) {
        this.sendReply(socket, ['!fatal', 'invalid credentials']);
        return;
      }
      
      setAuth(true);
      this.sendReply(socket, ['!done']);
      console.log(`[MockRouterOS] 🔐 User authenticated: ${username}`);
      return;
    }
